...
<?php
  ...
  @ $db = new mysqli('localhost', 'root', 'FuckThis1', 'iit');
  ...
  if ($havePost) {
    ...
    $title = htmlspecialchars(trim($_POST["titles"]));
    $year = htmlspecialchars(trim($_POST["years"]));

    $focusId = ''; // trap the first field that needs updating, better would be to save errors in an array

    if ($title == '') {
      $errors .= '<li> Title may not be blank</li>';
      if ($focusId == '') $focusId = '#titles';
    }
    if ($year == '') {
      $errors .= '<li>Year may not be blank</li>';
      if ($focusId == '') $focusId = '#years';
    }
    ...
    if ($errors != '') {
      ...
      if ($dbOk) {
        ...
        $titleForDb = trim($_POST["titles"]);
        $yearForDb = trim($_POST["years"]);
...
<form id="addForm" name="addForm" action="movies.php" method="post" onsubmit="return validate(this);">
  <fieldset>
    <div class="formData">
      ...
      <label class="field" for="title">Titles:</label>
      <div class="value"><input type="text" size="60" value="<?php if($havePost && $errors != '') { echo $title; } ?>" name="titles" id="titles"/></div>

      <label class="field" for="year">Year:</label>
      <div class="value"><input type="text" size="10" maxlength="10" value="<?php if($havePost && $errors != '') { echo $year; } ?>" name="years" id="years"/> <em>yyyy</em></div>
